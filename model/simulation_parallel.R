...
DPsolve = function(beta,m,Theta,Pricing,outCheby,code_dir){
  start = Sys.time()
  ...
  Astar0 <- foreach(
    ...
    errAstatus= 1
    mkpolicy(x0=matrix(c(0,Emax),ncol=2),beta=0,m=m,Theta=Theta,Pricing=Pricing,Chebygrid=outCheby$grid,Lambda=Lambda0,state=matrix(outCheby$grid[ii,],nrow=1),Emax=Emax,errAstatus=errAstatus,maxD=outCheby$maxD,maxB=outCheby$maxB,G=outCheby$G,K=outCheby$K,bound=outCheby$bound)
  }
  
  # (2) Contract Bellman equation for value function when applying myopic policies
  Lambda = Lambda1 = Lambda0
  iter1 = 0
  outtrans = statetrans(A=Astar0,Pricing=Pricing,Chebygrid=outCheby$grid,state=outCheby$grid,maxD=outCheby$maxD,maxB=outCheby$maxB,G=outCheby$G,K=outCheby$K,bound=outCheby$bound)
  ...
  while( max(abs(errA))>1.e-6 ){
    ...
    Astar1temp <- foreach(
      ...
      #ii=i
      Emax = min(apply(outCheby$grid,1,sum)[ii],m)
      errAstatus= max(abs(errA[c(ii,outCheby$pts+ii)]))
      mkpolicy(x0=c(500,Emax),beta=beta,m=m,Theta=Theta,Pricing=Pricing,Chebygrid=outCheby$grid,Lambda=Lambda,state=matrix(outCheby$grid[ii,],nrow=1),Emax=Emax,errAstatus=errAstatus,maxD=outCheby$maxD,maxB=outCheby$maxB,G=outCheby$G,K=outCheby$K,bound=outCheby$bound)
    }
    Astar1[index,] = Astar1temp
    errA = matrix(Astar-Astar1,ncol=2)
    Astar = Astar1
    
    # (3b) Bellman contraction-mapping
    iter1 = 0
    outtrans = statetrans(A=Astar1,Pricing=Pricing,Chebygrid=outCheby$grid,state=outCheby$grid,maxD=outCheby$maxD,maxB=outCheby$maxB,G=outCheby$G,K=outCheby$K,bound=outCheby$bound)
    util = waterutility(W=outtrans$W,E=outtrans$E,O=outtrans$O,m=m,Theta=Theta)
    while(max(abs(errV))>1.e-6){
      # Value Function Contraction
...
outV = output = NULL
for(hh in 1:2){
  outV[[hh]] = DPsolve(beta=beta,m=m[hh],Theta=Theta,Pricing=Pricing,outCheby=outCheby,code_dir=code_dir)
