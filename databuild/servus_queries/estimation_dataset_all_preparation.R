...
estimation_dataset_all <- read_parquet(paste0(working_data_dir, '/servus/processed/estimation_dataset_all.parquet'))
...
estimation_dataset_all <- estimation_dataset_all %>%
  ...
  select(id=account_id,
         ...
         tract_id,
         block_group_id,
         aspire_occupancy_status=occupancy_status,
         aspire_age=age,
         aspire_estimated_current_home_value=estimated_current_home_value,
         aspire_income=income,
         aspire_household_size=household_size,
         aspire_adult_count=adult_count,
         aspire_child_count=child_count,
         service_water,
         service_sewer,
         service_storm,
         rct_discount,
...
var_label(estimation_dataset_all$id) <- 'Household ID'
var_label(estimation_dataset_all$bill_date) <- 'Bill issue date'
var_label(estimation_dataset_all$due_date) <- 'Bill due date'
var_label(estimation_dataset_all$t) <- 'Relative time from RCT bill'
var_label(estimation_dataset_all$B_t) <- 'Current bill amount'
var_label(estimation_dataset_all$D_t) <- 'Unpaid debt'
var_label(estimation_dataset_all$F_t) <- 'Fees'
var_label(estimation_dataset_all$lag_F_t) <- 'Fees from previous bill'
var_label(estimation_dataset_all$O_t) <- 'Total amount due'
var_label(estimation_dataset_all$E_t) <- 'Total payment'
var_label(estimation_dataset_all$lag_w_t) <- 'Water use for the bill (in ccf)'
var_label(estimation_dataset_all$w_t) <- 'Water use for the next bill (in ccf)'
var_label(estimation_dataset_all$has_environment_discount) <- 'Received environmental discount'
var_label(estimation_dataset_all$shutoff_date) <- 'Water shutoff date'
var_label(estimation_dataset_all$reconnect_date) <- 'Water reconnect date'
var_label(estimation_dataset_all$account_status) <- 'Customer account status'
var_label(estimation_dataset_all$ufh) <- 'Below UFH flag'
var_label(estimation_dataset_all$fa_eligible) <- 'Eligible for financial assistance'
var_label(estimation_dataset_all$below_median_income) <- 'Income below median'
var_label(estimation_dataset_all$payment_plan) <- 'Enrolled in payment plan'
var_label(estimation_dataset_all$monthly_payment) <- 'On monthly installment billing'
var_label(estimation_dataset_all$linc_tier_type_at_bill) <- 'Financial assistance applied on the bill'
var_label(estimation_dataset_all$is_honored_citizen) <- 'Flag for honored citizen status'
var_label(estimation_dataset_all$is_rebill) <- 'Is bill a reissue'
var_label(estimation_dataset_all$first_bill) <- 'Bill was the first bill in the system'
var_label(estimation_dataset_all$cycle_code) <- 'Billing cycle code'
var_label(estimation_dataset_all$state_id) <- 'State FIPS code'
var_label(estimation_dataset_all$county_id) <- 'County FIPS code'
var_label(estimation_dataset_all$tract_id) <- 'Census tract'
var_label(estimation_dataset_all$block_group_id) <- 'Census block group'
var_label(estimation_dataset_all$service_water) <- 'Water service recipient'
var_label(estimation_dataset_all$service_sewer) <- 'Sewer service recipient'
var_label(estimation_dataset_all$service_storm) <- 'Stormwater service recipient'
var_label(estimation_dataset_all$rct_discount) <- 'RCT discount amount'
var_label(estimation_dataset_all$cleanriver_discount) <- 'Environmental discount amount'
var_label(estimation_dataset_all$linc_discount) <- 'Financial assistance discount amount'
var_label(estimation_dataset_all$crisis_voucher) <- 'Crisis voucher discount amount'

# Add census variables labels
for (var in census_variables) {
  var_label(estimation_dataset_all[[paste0('census_', var)]]) <- 
