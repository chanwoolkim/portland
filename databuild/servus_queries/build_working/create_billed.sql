...
ordered_bills AS (
  SELECT 
    *,
    LEAD(source_code, 1) OVER (PARTITION BY account_id ORDER BY bill_date) AS next_source_code,
    LEAD(due_date, 1) OVER (PARTITION BY account_id ORDER BY bill_date) AS next_due_date,
    COALESCE(LEAD(amount, 1) OVER (PARTITION BY account_id ORDER BY bill_date), 0) AS next_amount,
    LEAD(source_code, 2) OVER (PARTITION BY account_id ORDER BY bill_date) AS next_next_source_code,
    LEAD(due_date, 2) OVER (PARTITION BY account_id ORDER BY bill_date) AS next_next_due_date,
    COALESCE(LEAD(amount, 2) OVER (PARTITION BY account_id ORDER BY bill_date), 0) AS next_next_amount
  FROM filtered_bills
  WHERE bill_num = 1
),
...
SELECT
  ...
  COALESCE(LEAD(bill_date) OVER (PARTITION BY quarterly_bill.account_id ORDER BY bill_date ASC), CURRENT_DATE()) AS next_bill_date,
...
FROM quarterly_bill
  JOIN account 
  ON account.account_id = quarterly_bill.account_id
ORDER BY quarterly_bill.account_id, quarterly_bill.bill_date;